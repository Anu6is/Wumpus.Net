...
namespace Wumpus.Serialization
{
    internal class ConverterCollection
    {
        ...
        private object Create(Serializer serializer, Type type, PropertyInfo propInfo = null, bool throwOnNotFound  = true)
        {
            ...
            {
                converterType = FindGenericConverterType(type, _mappedGenericTypes, typeInfo, propInfo);
                if (converterType != null)
                    return BuildConverter(converterType, serializer);
            }

            //Normal converters (bool -> BooleanPropertyConverter)
            converterType = FindConverterType(type, _types, typeInfo, propInfo);
            if (converterType != null)
                return BuildConverter(converterType, serializer);

            //Generic converters (Model -> ObjectPropertyConverter<Model>)
            converterType = FindGenericConverterType(type, _genericTypes, typeInfo, propInfo);
            if (converterType != null)
                return BuildConverter(converterType, serializer);
        ...
        private object BuildConverter(Type converterType, Serializer serializer)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                if (parameters[i].Name == "innerConverter")
                    args[i] = serializer.GetConverter(converterTypeInfo.GenericTypeArguments[0], null);
        ...
        private Type FindConverterType(Type type, Dictionary<Type, ConverterTypeCollection> collection, TypeInfo typeInfo, PropertyInfo propInfo = null)
        {
            if (collection.TryGetValue(type, out var converters))
                return FindConverterType(converters, typeInfo, propInfo);
            return null;
        }
        private Type FindConverterType(ConverterTypeCollection converters, TypeInfo typeInfo, PropertyInfo propInfo = null)
        {
        ...
        private Type FindGenericConverterType(Type type, Dictionary<Type, GenericConverterTypeCollection> collection, TypeInfo typeInfo, PropertyInfo propInfo = null)
        {
            ...
            if (innerType != null)
            {
                if (collection.TryGetValue(openGenericType, out var converters))
                    return FindGenericConverterType(innerType, converters, typeInfo, propInfo);
            }
            return null;
        }
        private Type FindGenericConverterType(Type innerType, GenericConverterTypeCollection converters, TypeInfo typeInfo, PropertyInfo propInfo = null)
        {
            for (int i = 0; i < converters.Conditionals.Count; i++)
            {
                if (converters.Conditionals[i].Condition(typeInfo, propInfo))

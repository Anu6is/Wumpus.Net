...
namespace System.Text.Parsing
{
    ...
    {
        const int _stackBufferSize = 128;
        ...
        public static bool TryParseUInt64<T>(this T bufferSequence, out ulong value, out int consumed) where T : ISequence<ReadOnlyBuffer<byte>>
        {
            ...
            if (!bufferSequence.TryGet(ref position, out ReadOnlyBuffer<byte> first))
            {
                return false;
            }
            ...
            if (!bufferSequence.TryGet(ref position, out ReadOnlyBuffer<byte> second))
            {
            ...
            unsafe
            {
                if (first.Length < _stackBufferSize) {
                    var data = stackalloc byte[_stackBufferSize];
                    var destination = new Span<byte>(data, _stackBufferSize);
                    ...
                    while (free.Length > 0)
                    {
                        if (bufferSequence.TryGet(ref position, out ReadOnlyBuffer<byte> next))
                        {
                    ...
                    combinedSpan = destination.Slice(0, _stackBufferSize - free.Length);
                    ...
                    if (PrimitiveParser.InvariantUtf8.TryParseUInt64(combinedSpan, out value, out consumed)) {
                        if(consumed < combinedSpan.Length || combinedSpan.Length < _stackBufferSize) {
                            return true;
                        }
                    }
                }
        ...
        public static bool TryParseUInt32<T>(this T bufferSequence, out uint value, out int consumed) where T : ISequence<ReadOnlyBuffer<byte>>
        {
            ...
            unsafe
            {
                if (first.Length < _stackBufferSize) {
                    ...
                    while (free.Length > 0)
                    {
                        if (bufferSequence.TryGet(ref position, out var next))
                        {
                            if (next.Length > free.Length) next = next.Slice(0, free.Length);
                            next.CopyTo(free);
                            free = free.Slice(next.Length);
